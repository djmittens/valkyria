#pragma once

#include "common.h"
#include "memory.h"

#include <stddef.h>

#include <openssl/bio.h>
#include <openssl/conf.h>
#include <openssl/err.h>
#include <openssl/ssl.h>

typedef struct {
  SSL_CTX *ssl_ctx;
} valk_aio_ssl_ctx_t;

typedef struct valk_aio_ssl_t {
  SSL *ssl;
  BIO *read_bio;
  BIO *write_bio;

  // Bytes we are waiting to send.
  valk_buffer_t unencrypted;

  // Bytes we are waiting to receive.
  valk_buffer_t encrypted;

  // Bytes we pass to callbacks.
  valk_buffer_t staging;

} valk_aio_ssl_t;

valk_err_e valk_aio_ssl_server_init(valk_aio_ssl_ctx_t *ctx,
                                    const char *keyfile, const char *certfile);
void valk_aio_ssl_accept(valk_aio_ssl_t *ssl, valk_aio_ssl_ctx_t *ctx);

void valk_aio_ssl_close(valk_aio_ssl_t *ssl);
void valk_aio_ssl_ctx_close(valk_aio_ssl_ctx_t *ctx);

valk_err_e valk_aio_ssl_on_read(valk_aio_ssl_t *ssl,
                                void* data,
                                void(onRead)(void*, const valk_buffer_t *));


valk_err_e valk_aio_ssl_encrypt(valk_aio_ssl_t *ssl);

void valk_aio_ssl_print_state(valk_aio_ssl_t *ssl);
void valk_aio_ssl_print_error(valk_aio_ssl_t *ssl);
