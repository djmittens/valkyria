...
static valk_lval_t* valk_builtin_aio_semaphore(valk_lenv_t* e, valk_lval_t* a) {
  UNUSED(e);
  if (valk_lval_list_count(a) != 2) {
    return valk_lval_err("aio/semaphore: expected 2 arguments (aio permits)");
  }
  valk_lval_t *aio_ref = valk_lval_list_nth(a, 0);
  valk_lval_t *permits_arg = valk_lval_list_nth(a, 1);

  if (LVAL_TYPE(aio_ref) != LVAL_REF || strcmp(aio_ref->ref.type, "aio_system") != 0) {
    return valk_lval_err("aio/semaphore: first argument must be aio system");
  }
  if (LVAL_TYPE(permits_arg) != LVAL_NUM) {
    return valk_lval_err("aio/semaphore: second argument must be a number");
  }
  if (permits_arg->num < 1) {
    return valk_lval_err("aio/semaphore: permits must be >= 1");
  }

  valk_aio_system_t *sys = aio_ref->ref.ptr;

  valk_semaphore_t *sem = valk_region_alloc(&sys->system_region, sizeof(valk_semaphore_t));
  if (!sem) {
    return valk_lval_err("aio/semaphore: allocation failed");
  }

  sem->magic = VALK_SEMAPHORE_MAGIC;
  sem->sys = sys;
  sem->max_permits = (int)permits_arg->num;
  sem->available = (int)permits_arg->num;
  sem->waiting = NULL;
  sem->waiting_count = 0;
  sem->waiting_capacity = 0;

  return valk_lval_ref("semaphore", sem, NULL);
}

static valk_lval_t* valk_builtin_aio_semaphore_acquire(valk_lenv_t* e, valk_lval_t* a) {
  if (valk_lval_list_count(a) != 1) {
    return valk_lval_err("aio/semaphore-acquire: expected 1 argument (semaphore)");
  }
  valk_lval_t *sem_ref = valk_lval_list_nth(a, 0);

  if (LVAL_TYPE(sem_ref) != LVAL_REF || strcmp(sem_ref->ref.type, "semaphore") != 0) {
    return valk_lval_err("aio/semaphore-acquire: argument must be a semaphore");
  }

  valk_semaphore_t *sem = sem_ref->ref.ptr;
  if (!sem || sem->magic != VALK_SEMAPHORE_MAGIC) {
    return valk_lval_err("aio/semaphore-acquire: invalid semaphore");
  }

  valk_async_handle_t *handle = valk_async_handle_new(sem->sys, e);
  if (!handle) {
    return valk_lval_err("aio/semaphore-acquire: allocation failed");
  }

  if (sem->available > 0) {
    sem->available--;
    handle->status = VALK_ASYNC_COMPLETED;
    atomic_store_explicit(&handle->result, valk_lval_sym(":acquired"), memory_order_release);
    return valk_lval_handle(handle);
  }

  handle->status = VALK_ASYNC_RUNNING;

  if (sem->waiting_count >= sem->waiting_capacity) {
    u64 new_cap = sem->waiting_capacity == 0 ? 8 : sem->waiting_capacity * 2;
    valk_async_handle_t **new_waiting = valk_region_alloc(&sem->sys->system_region,
                                                          sizeof(valk_async_handle_t*) * new_cap);
    if (!new_waiting) {
      handle->status = VALK_ASYNC_FAILED;
      atomic_store_explicit(&handle->error, valk_lval_err("aio/semaphore-acquire: allocation failed"), memory_order_release);
      return valk_lval_handle(handle);
    }
    if (sem->waiting) {
      for (u64 i = 0; i < sem->waiting_count; i++) {
        new_waiting[i] = sem->waiting[i];
      }
    }
    sem->waiting = new_waiting;
    sem->waiting_capacity = new_cap;
  }

  sem->waiting[sem->waiting_count++] = handle;

  return valk_lval_handle(handle);
}

static valk_lval_t* valk_builtin_aio_semaphore_release(valk_lenv_t* e, valk_lval_t* a) {
  UNUSED(e);
  if (valk_lval_list_count(a) != 1) {
    return valk_lval_err("aio/semaphore-release: expected 1 argument (semaphore)");
  }
  valk_lval_t *sem_ref = valk_lval_list_nth(a, 0);

  if (LVAL_TYPE(sem_ref) != LVAL_REF || strcmp(sem_ref->ref.type, "semaphore") != 0) {
    return valk_lval_err("aio/semaphore-release: argument must be a semaphore");
  }

  valk_semaphore_t *sem = sem_ref->ref.ptr;
  if (!sem || sem->magic != VALK_SEMAPHORE_MAGIC) {
    return valk_lval_err("aio/semaphore-release: invalid semaphore");
  }

  if (sem->waiting_count > 0) {
    valk_async_handle_t *waiter = sem->waiting[0];
    for (u64 i = 1; i < sem->waiting_count; i++) {
      sem->waiting[i - 1] = sem->waiting[i];
    }
    sem->waiting_count--;

    valk_async_handle_complete(waiter, valk_lval_sym(":acquired"));
  } else {
    if (sem->available < sem->max_permits) {
      sem->available++;
    }
  }
...
void valk_register_async_handle_builtins(valk_lenv_t *env) {
  ...
  valk_lenv_put_builtin(env, "aio/semaphore", valk_builtin_aio_semaphore);
  valk_lenv_put_builtin(env, "aio/semaphore-acquire", valk_builtin_aio_semaphore_acquire);
  valk_lenv_put_builtin(env, "aio/semaphore-release", valk_builtin_aio_semaphore_release);
}
