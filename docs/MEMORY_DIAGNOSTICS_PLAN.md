...
**Current**: Binary bitmap (1 bit per slot: used/free)
```json
{"name":"handles","bitmap":"ff00ff00...","total":500,"used":47}
```

**New**: Per-slot state array with attribution
```json
{
  "name": "handles",
  "total": 500,
  "slots": [
    {"state": "active", "owner": ":8080", "age_ms": 1234},
    {"state": "idle", "owner": ":8080", "age_ms": 5678},
    {"state": "free"},
    {"state": "closing", "owner": ":9090", "age_ms": 100},
    ...
  ],
  "summary": {
    "by_state": {"active": 38, "idle": 7, "closing": 2, "free": 453},
    "by_owner": {":8080": 32, ":9090": 12, ":3000": 3}
  }
}
```
...
Full per-slot metadata is verbose. Use compact encoding:
...
```json
{
  ...
  "total": 500,
  "used": 47,
  "states": "AAIAFAAFAAI...",   // 1 char per slot: A=active, I=idle, C=closing, F=free
  "owners": [0,0,0,1,0,0,2,...], // Owner index per used slot (0=:8080, 1=:9090, etc)
  "owner_map": [":8080", ":9090", ":3000"],
  "summary": {
    "by_state": {"A": 38, "I": 7, "C": 2},
    "by_owner": {"0": 32, "1": 12, "2": 3}
  }
}
```
...
1. **Track connection owner**: Store server/client pointer in handle struct
2. **Track connection state**: Already exists in `valk_aio_metrics_t` counters, but need per-handle state
3. **Expose via accessor**: `valk_aio_handle_get_state()`, `valk_aio_handle_get_owner()`
...
1. **Phase 2a: Backend State Tracking**
   - Add `valk_conn_state_e` enum to `aio.h`
   - Add `valk_handle_diag_t` to handle struct
   - Update lifecycle functions to set state
   - Add owner registry and registration

2. **Phase 2b: Diagnostics Collection**
   - Modify `valk_slab_snapshot_t` for per-slot data
   - Implement `slab_to_slot_diag()`
   - Update `valk_mem_snapshot_to_sse()` for compact format
   - Test with manual slab inspection

3. **Phase 2c: Frontend Grid**
   - Add `renderConnectionGrid()` function
   - Update `handleMemoryUpdate()` for new format
   - Add state color CSS

4. **Phase 2d: Dashboard Restructure**
   - Move HTTP Servers section under AIO
   - Add owner breakdown visualization
