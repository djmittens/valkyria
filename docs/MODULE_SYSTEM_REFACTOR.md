...
Three-pass load in `eval_loaded_ast()` (`src/builtins_io.c:186-279`):
...
1. **Pass 1 — macro expansion.** Every `(macro NAME BODY)` in the file
   ...
   known macro. This is global, not module-scoped (`src/builtins_io.c:195-204`).
2. **Pass 1.5 — child pre-registration.** Walks the AST for nested
   ...
   (`src/builtins_io.c:228-230`, fix from 5cf0118).
3. **Pass 2 — FQN rewrite.** `valk_module_rewrite` (`src/macro.c:289-312`)
   ...
   - Qualified reference `analysis/foo` → canonicalized via `resolve_qualified`
     (`src/macro.c:162-185`), which walks up the module tree to find the
     first ancestor with a child named `analysis`.

Data structures (`src/module.h:8-19`): a global tree of `valk_module_t`,
each with a list of `def_names` and `def_vals`. Current module is TLS
(`g_current_mod`, `src/module.c:7-8`).
...
`scripts/lsp/nav.valk` calls `(analysis/line-col->offset …)`. Today the
...
- (a) Require every cross-module reference to be fully qualified
  ...
  change for ~30 call sites in `scripts/lsp/*.valk`.
...
`(macro ...)` forms evaluate in the global macro env (`src/builtins_io.c:195-198`),
...
qualifying them (`src/macro.c:139-146, 202-208`). If qualification moves
...
2. LSP reader callback at `src/builtins_pipe.c:340` invokes the dispatch;
...
4. Parent test waits `MSG_TIMEOUT_MS = 5000` for a response that never
   comes (`test/lsp/test_lsp_integration.c:15`).
...
1. In `src/builtins_pipe.c:340`, if `LVAL_TYPE(result) == LVAL_ERR`, emit a
   JSON-RPC error with the error message to stdout. Fails loudly.
2. In `scripts/lsp/lsp.valk:192` (`lsp/on-message`) and :79
   (`lsp/dispatch-request`), add `stderr/write` of method name.
3. Provide a standalone repro without the C harness:
   `./build/valk scripts/lsp/main.valk <<<$(printf 'Content-Length: 79\\r\\n\\r\\n{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"capabilities":{}}}')`
...
- **Option A**: accept the breaking change — require every cross-file
  ...
  rewrites `scripts/lsp/*.valk`. Risks: third-party code, if any.
