...
See [COVERAGE_REQUIREMENTS.md](../../coverage/docs/COVERAGE_REQUIREMENTS.md) for detailed tier requirements.
...
Located in `runtime/test/stress/`:

```bash
# Run stress tests (not part of regular test suite)
./build/valk runtime/test/stress/test_gc_stress.valk
./build/valk runtime/test/stress/test_networking_stress.valk
```
