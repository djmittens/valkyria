...
void test_env_put_marks_escaping(VALK_TEST_ARGS()) {
  ...
  VALK_TEST_ASSERT(LVAL_ESCAPES(val), "Value should be marked as escaping after env put");
  VALK_PASS();
}
...
void test_lambda_captures_escape(VALK_TEST_ARGS()) {
  ...
  // Initially not marked as escaping
  VALK_TEST_ASSERT(!LVAL_ESCAPES(formals), "Formals should not escape initially");
  ...
  VALK_TEST_ASSERT(LVAL_ESCAPES(body), "Body should escape after lambda creation");
...
void test_zero_copy_frozen_heap_values(VALK_TEST_ARGS()) {
  ...
  VALK_TEST_ASSERT(LVAL_IS_FROZEN(original),
...
void test_scratch_frozen_values_copied(VALK_TEST_ARGS()) {
  ...
  VALK_TEST_ASSERT(LVAL_IS_FROZEN(scratch_val),
                   "Value should be frozen");

  // Intern into global - should make a copy (not alias)
  valk_lval_t* interned = valk_intern(env, scratch_val);

  VALK_TEST_ASSERT(interned != scratch_val,
                   "Scratch values should be copied, not aliased (even if frozen)");
  // Note: Both arenas have type VALK_ALLOC_ARENA, so both will be marked LVAL_ALLOC_SCRATCH
  // The distinction between "global" and "scratch" arena is semantic, not in the allocator type
  VALK_TEST_ASSERT(LVAL_ALLOC(interned) == LVAL_ALLOC_SCRATCH,
