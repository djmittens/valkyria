...
//   VALK_LSP_ARGS — if set to "main.valk", run valk + scripts/lsp/main.valk
...
// exec as `<bin_path> scripts/lsp/main.valk` (for when bin is the interpreter).
...
static lsp_t lsp_spawn(const char *bin_path, int run_main_valk,
  ...
  if (pid == 0) {
    ...
    if (run_main_valk)
      execlp(bin_path, bin_path, "scripts/lsp/main.valk", NULL);
    else
...
static void run_profile(const char *label, const char *bin_path,
  ...
  fprintf(stderr, "  bin:        %s%s\n", bin_path,
          run_main_valk ? " scripts/lsp/main.valk" : "");
