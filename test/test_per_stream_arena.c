...
void test_stream_arena_isolation(VALK_TEST_ARGS()) {
  ...
  VALK_TEST_ASSERT(item1 != nullptr, "First arena item should be allocated");
  VALK_TEST_ASSERT(item2 != nullptr, "Second arena item should be allocated");
  ...
  VALK_WITH_ALLOC((valk_mem_allocator_t*)arena1) {
    ...
    VALK_TEST_ASSERT(p != nullptr, "Arena1 allocation should succeed");
  }

  // Arena2 should still have full capacity
  VALK_WITH_ALLOC((valk_mem_allocator_t*)arena2) {
    void *p = valk_mem_alloc(3000);
    VALK_TEST_ASSERT(p != nullptr, "Arena2 should have space independent of arena1");
  }
  ...
  VALK_WITH_ALLOC((valk_mem_allocator_t*)arena2) {
    ...
    VALK_TEST_ASSERT(p != nullptr, "Arena2 should still work after arena1 released");
  }
...
void test_stream_arena_reuse(VALK_TEST_ARGS()) {
  ...
  VALK_WITH_ALLOC((valk_mem_allocator_t*)arena) {
    void *p = valk_mem_alloc(3000);
    VALK_TEST_ASSERT(p != nullptr, "Initial allocation should succeed");
  }
  ...
  valk_slab_item_t *item2 = valk_slab_aquire(slab);
  VALK_TEST_ASSERT(item2 != nullptr, "Reacquire should succeed");
  ...
  VALK_WITH_ALLOC((valk_mem_allocator_t*)arena) {
    ...
    VALK_TEST_ASSERT(p != nullptr, "Reused arena should have full capacity");
  }
...
void test_stream_arena_exhaustion(VALK_TEST_ARGS()) {
  ...
  VALK_TEST_ASSERT(item3 == nullptr, "Third acquire should fail (pool exhausted)");

  // Release one, then acquire should work
  valk_slab_release(slab, item1);
  item3 = valk_slab_aquire(slab);
  VALK_TEST_ASSERT(item3 != nullptr, "Acquire after release should succeed");
...
void test_stream_arena_sequential(VALK_TEST_ARGS()) {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    VALK_TEST_ASSERT(item != nullptr, "Request %d: arena acquire should succeed", i);
    ...
    VALK_WITH_ALLOC((valk_mem_allocator_t*)arena) {
      ...
      VALK_TEST_ASSERT(data != nullptr, "Request %d: data allocation should succeed", i);
      memset(data, 'X', 1024);
    }
...
void test_stream_arena_concurrent(VALK_TEST_ARGS()) {
  ...
  for (int i = 0; i < NUM_STREAMS; i++) {
    ...
    VALK_TEST_ASSERT(items[i] != nullptr, "Stream %d arena should be acquired", i);
  ...
  for (int i = 0; i < NUM_STREAMS; i++) {
    VALK_WITH_ALLOC((valk_mem_allocator_t*)arenas[i]) {
      ...
      VALK_TEST_ASSERT(data != nullptr, "Stream %d data allocation should succeed", i);
...
void test_stream_arena_no_memory_growth(VALK_TEST_ARGS()) {
  ...
  for (int i = 0; i < 10000; i++) {
    ...
    VALK_WITH_ALLOC((valk_mem_allocator_t*)arena) {
      ...
      VALK_TEST_ASSERT(p != nullptr, "Iteration %d allocation should succeed", i);
    }
...
void test_stream_arena_size_adequate(VALK_TEST_ARGS()) {
  ...
  valk_slab_item_t *item = valk_slab_aquire(slab);
  VALK_TEST_ASSERT(item != nullptr, "Should acquire arena");
  ...
  VALK_WITH_ALLOC((valk_mem_allocator_t*)arena) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      VALK_TEST_ASSERT(hdr != nullptr, "Pseudo-header %d should allocate", i);
    }
    ...
    for (int i = 0; i < 20; i++) {
      ...
      VALK_TEST_ASSERT(name != nullptr && value != nullptr, "Header %d should allocate", i);
    }

    // Request body
    char *body = valk_mem_alloc(8192);
    VALK_TEST_ASSERT(body != nullptr, "Request body should allocate");

    // Qexpr overhead
    char *qexpr = valk_mem_alloc(4096);
    VALK_TEST_ASSERT(qexpr != nullptr, "Qexpr should allocate");

    // Response body
    char *response = valk_mem_alloc(16384);
    VALK_TEST_ASSERT(response != nullptr, "Response body should allocate");
  }
