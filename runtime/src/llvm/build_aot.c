...
// The dispatch table TU is compiled on the deployment machine, which has no
// runtime headers. It only needs the layout of valk_aot_entry_t (image.h):
// {const char *name; valk_lval_t *(*fn)(valk_lenv_t *);} — mirrored here
// with local opaque types, identical by layout.
static void write_dispatch_c(FILE *f, aot_entry_t *entries, size_t n) {
  ...
  for (size_t i = 0; i < n; i++) {
    fprintf(f, "extern valk_shim_lval_t *%s(valk_shim_lenv_t *);\n",
            entries[i].name);
  }
  fprintf(f, "\n");
  if (n > 0) {
    fprintf(f, "static const valk_shim_aot_entry_t _table[] = {\n");
    for (size_t i = 0; i < n; i++) {
      fprintf(f, "  {\"%s\", %s},\n", entries[i].name, entries[i].name);
    }
    fprintf(f, "};\n");
    fprintf(f, "const valk_shim_aot_entry_t *const valk_aot_table = _table;\n");
  } else {
    fprintf(f, "const valk_shim_aot_entry_t *const valk_aot_table = NULL;\n");
  }
