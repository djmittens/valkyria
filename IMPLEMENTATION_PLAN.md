...
- [ ] **parser.c** - ~~75.1%~~ ~~77.0%~~ ~~81.0%~~ ~~81.7%~~ ~~83.3%~~ ~~83.9%~~ 85.9% line / ~~50.0%~~ ~~51.1%~~ ~~55.1%~~ ~~55.8%~~ ~~56.8%~~ ~~57.5%~~ 59.9% branch - INCREMENTALLY IMPROVING
  ...
  - Added test/test_parser_coverage_gaps.valk (50 tests) covering quasiquote nil, unquote wrong args, splice top-level error, ctx multi-body paths, varargs & errors, GC builtins (gc-stats, gc-collect, gc-threshold-pct), math builtins (unary negation, type errors), list builtins (init, last), parser error paths (unterminated strings), escape sequences, type predicates
  ...
  - Remaining uncovered: coverage builtins (only in VALK_COVERAGE builds), HTTP/2 server builtins (require integration tests), continuation handling thunk paths, penv builtin, debug/trace logging paths
